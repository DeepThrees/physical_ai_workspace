...
## Phase 8 — 로컬 SLM(Ollama) 도입 테스트: 완벽한 실패

Phase 7에서 도출한 클라우드 Latency 한계를 극복하기 위해, `gemini-2.5-flash`를 로컬 SLM(`Ollama` + `qwen2.5:3b`)으로 교체하고 실시간 자율주행을 재검증했다.

- [X] `agents/autonomous_agent.py`를 Ollama 로컬 추론(`http://localhost:11434/api/generate`)으로 전환
  - 네트워크 왕복 지연은 **0.1초대**로 줄어 Phase 7의 3~4초 제어 주기 병목은 해소됨
  - `lidar_translator.py` 쓰기 간격도 `0.5s`(2Hz) → `0.05s`(20Hz)로 낮춰 최신 센서 요약을 즉각 반영하도록 조정
  - 조향 부호, 직진 강제, Few-Shot 예시, 3상태 JSON 고정 등 SYSTEM_PROMPT / user prompt를 반복 강화
- [X] 실시간 시뮬레이터 자율주행 제어: **완전히 실패**
  - **공간 방향 부호(+/-) 혼동:** 좌측이 막혀도 `+`로 꺾거나, 우측이 넓어도 `-`를 내지 않음. 프롬프트에 "좌측=양수 / 우측=음수"를 명시해도 반복적으로 무시
  - **프롬프트 규칙 무시:** 전방 2.0m / 1.2m 이상이면 직진(`steering_angle: 0.0`) 같은 단순 규칙조차 지키지 않음
  - **JSON 포맷 붕괴(Hallucination):** 필수 키 누락, 스키마 밖 필드, 자연어가 섞인 깨진 JSON. 스키마를 프롬프트에 박아도 출력이 붕괴
  - **기본 기동 불가:** 뻥 뚫린 직선 구간에서의 직진, 느린 코너링조차 수행하지 못하고 **무한 벽 충돌**을 반복
- [X] 구조적 결함 확인 — Django Safety의 허상
  - 현재 Safety Gatekeeper는 **JSON 문법과 숫자 범위(속도/조향 clamp)만 검사하는 껍데기**
  - 실제 물리적 충돌을 막아주는 AEB(Automatic Emergency Braking) / TTC 기반 하드 스톱이 파이프라인에 없음
  - SLM이 틀린 조향을 내도 티켓은 `APPROVED` → ROS2 `/drive`로 그대로 전달되어 벽에 반복 충돌

### 핵심 교훈

**LLM/SLM을 End-to-End 실시간 물리 제어에 단독으로 쓰는 기존 아키텍처는 전면 폐기한다.**

Latency를 없애도 SLM은 (1) 공간 좌표 부호, (2) 규칙 준수, (3) 구조화된 출력 안정성을 보장하지 못한다. 다음 단계는 LLM을 고수준 의도/플랜 계층에만 두고, 저수준 조향·제동은 규칙 기반 또는 고전 제어(MPC 등)가 담당하는 하이브리드로 재설계해야 한다.
